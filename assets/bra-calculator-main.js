  ...
  for (var i = 0; i < QUICK_FILL_DATA.length; i++){
    (function(item){
      ...
      btn.addEventListener('click', function(){
        ...
        if (ub) {
          ub.value = ubVal;
          /* 触发 input 事件 → 联动 Live Preview + 输入框高亮 */
          ub.dispatchEvent(new Event('input', { bubbles: true }));
        }
        if (b) {
          b.value = bVal;
...
function renderSisterComparator(result){
  ...
  try {
    ...
    /* 渲染 7 张姐妹卡 (extended + sister + you + sister + extended) */
    ...
    if (sistersEl){
      ...
      /* 移动端 (<= 540px) 强制 inline grid 布局, 避免 CSS 顺序覆盖问题 */
      if (window.innerWidth <= 540){
        applyMobileSisterGrid(sistersEl);
      }
...
function buildSisterCardHtml(ss, i, total, userOriginal, withSpin){
  ...
  /* roleLbl 保持简洁, 配合 offset 标签传达完整信息:
     - YOU (center)  + ▼
     - SISTER (邻近)  + optional offset
     - EXTENDED (远端) + "±3 sizes" 标签 */
  var roleLbl = ss.primary ? 'YOU' : (role === 'sister' ? 'SISTER' : 'EXTENDED');

  /* ─── 关键: 姐妹尺码的体积显示 ───
     姐妹尺码定义 = 相同罩杯体积(不同底围+不同杯字母)
     所以每张卡都必须显示相同的体积数字(用户自身的体积),
     这是 sister size 的核心定义 — 体积相等, band 改变
     微小差异会让用户误以为"姐妹尺码体积不同" */
  var userVol = (userOriginal && BC.CUP_VOLUME_ML) ? (BC.CUP_VOLUME_ML[userOriginal.cup] || 380) : 380;
  /* 全部姐妹尺码都显示 userVol — 这是"等体积"原则 */
  var vol = userVol;
  /* 显示: 整数 + 单位 (mL) */
  var bandCm = (ss.band * 2.54).toFixed(1).replace(/\.0$/, '');
  ...
  /* ─── 简化版换算 — 只显示 US + EU 底围数字 (杯字母已在卡顶显示) ───
     关键: 整张卡的换算行宽度 = 2 块 + 间距, 只显示底围数字 (28 / 60)
     杯字母从卡顶推断, 避免文字溢出 */
  var conv = BC.getSisterSizeConversion ? BC.getSisterSizeConversion(ss.band, ss.cup) : null;
  var convHtml = '';
  if (conv){
    /* 从 '60F' 提取 '60' 底围部分 */
    var usBand = (conv.us.match(/^\d+/) || [''])[0];
    var euBand = (conv.eu.match(/^\d+/) || [''])[0];
    convHtml =
      '<span class="bc-compass-sister-conversion">' +
        '<span class="bc-compass-sister-region bc-compass-sister-region-us" title="US ' + conv.us + '">' +
          '<span class="bc-compass-sister-region-tag">US</span>' +
          '<span class="bc-compass-sister-region-num">' + usBand + '</span>' +
        '</span>' +
        '<span class="bc-compass-sister-region" title="EU ' + conv.eu + '">' +
          '<span class="bc-compass-sister-region-tag">EU</span>' +
          '<span class="bc-compass-sister-region-num">' + euBand + '</span>' +
        '</span>' +
      '</span>';
  }

  /* ─── extended 卡加 offset 辅助文字 (内联, 体积下方) ───
     关键: 7 张卡 -3..+3 offset, 显示真实偏移 (3/2/1/0/1/2/3 sizes from you)
     比 "±1 cup" 准确, 用户能一眼看出离自己多近 */
  var comfort = '';
  if (role === 'extended' && !isUserOriginal){
    /* 每个 band step = 2 inches = 1 size
       7 张卡 -3..+3, 但 band 是 2 inches/step
       28DDD band=28 vs 34, 差 -6 inches = -3 sizes (正确) */
    var offset = Math.abs(ss.band - userOriginal.band) / 2;
    var adjLabel = (offset === 1) ? 'size' : 'sizes';
    comfort = '<span class="bc-compass-sister-comfort" title="' + offset + ' ' + adjLabel + ' from your band ' + userOriginal.band + '">±' + offset + ' ' + adjLabel + '</span>';
  } else if (isUserOriginal && !ss.primary){
    comfort = '<span class="bc-compass-sister-comfort">your size</span>';
  }
  ...
  /* tap-to-switch 提示 — hover 时显示, 引导用户点击 */
  var tapHint = (!ss.primary) ? '<span class="bc-compass-sister-tap-hint">tap to switch</span>' : '';
  ...
  return '<button type="button" class="bc-compass-sister bc-compass-sister-' + role + clickable + '"' +
         ...
         '<span class="bc-compass-sister-band">' + bandCm + ' cm</span>' +
         convHtml + comfort + origMark + tapHint +
         '</button>';
}

/* applyMobileSisterGrid — 移动端强制 inline grid 布局
   原因: CSS 中 540px 媒体查询虽存在, 但因 main.css 中多个 540px 规则相互覆盖,
         导致 display:grid 在某些情况下失效. 用 JS 强制 inline style 绕过.
   新设计: 顶部 = role + check 同行, 中部 = size 居中, 下部 = volume/band/conversion/comfort 居中 */
function applyMobileSisterGrid(sistersEl){
  if (!sistersEl) return;
  sistersEl.style.setProperty('display', 'grid', 'important');
  sistersEl.style.setProperty('grid-template-columns', '1fr', 'important');
  sistersEl.style.setProperty('gap', '8px', 'important');
  sistersEl.style.setProperty('padding', '0 4px', 'important');
  var cards = sistersEl.querySelectorAll('.bc-compass-sister');
  for (var i = 0; i < cards.length; i++){
    var card = cards[i];
    card.style.setProperty('display', 'grid', 'important');
    card.style.setProperty('grid-template-columns', '1fr auto', 'important');
    card.style.setProperty('grid-template-rows', 'auto auto auto auto auto', 'important');
    ...
    card.style.setProperty('gap', '3px 8px', 'important');
    card.style.setProperty('border-radius', '10px', 'important');

    var role = card.querySelector('.bc-compass-sister-role');
    if (role){
      role.style.setProperty('grid-column', '1', 'important');
      role.style.setProperty('grid-row', '1', 'important');
      role.style.setProperty('align-self', 'center', 'important');
      role.style.setProperty('justify-self', 'start', 'important');
      role.style.setProperty('font-size', '0.46rem', 'important');
      role.style.setProperty('font-weight', '700', 'important');
      role.style.setProperty('letter-spacing', '0.16em', 'important');
      role.style.setProperty('min-width', '0', 'important');
      role.style.setProperty('text-align', 'left', 'important');
      role.style.setProperty('color', '#b89580', 'important');
      role.style.setProperty('margin', '0', 'important');
      role.style.setProperty('padding', '0', 'important');
    }
    var check = card.querySelector('.bc-compass-sister-check');
    if (check){
      check.style.setProperty('grid-column', '2', 'important');
      check.style.setProperty('grid-row', '1', 'important');
      check.style.setProperty('align-self', 'center', 'important');
      check.style.setProperty('justify-self', 'end', 'important');
      check.style.setProperty('width', '24px', 'important');
      check.style.setProperty('height', '24px', 'important');
      check.style.setProperty('font-size', '0.85rem', 'important');
      check.style.setProperty('position', 'static', 'important');
      check.style.setProperty('margin', '0', 'important');
    }
    if (card.classList.contains('bc-compass-sister-you')){
      card.style.setProperty('position', 'relative', 'important');
      var youPointer = card.querySelector('.bc-compass-sister-pointer');
      if (youPointer) {
        youPointer.style.setProperty('display', 'block', 'important');
        youPointer.style.setProperty('position', 'absolute', 'important');
        youPointer.style.setProperty('top', '6px', 'important');
        youPointer.style.setProperty('right', '10px', 'important');
        youPointer.style.setProperty('font-size', '0.85rem', 'important');
        youPointer.style.setProperty('color', '#b87a5a', 'important');
        youPointer.style.setProperty('margin', '0', 'important');
        youPointer.style.setProperty('z-index', '3', 'important');
      }
      var youCheck = card.querySelector('.bc-compass-sister-check');
      if (youCheck) youCheck.style.setProperty('display', 'none', 'important');
    }
    var size = card.querySelector('.bc-compass-sister-size');
    if (size){
      size.style.setProperty('grid-column', '1 / -1', 'important');
      size.style.setProperty('grid-row', '2', 'important');
      size.style.setProperty('font-size', '1.2rem', 'important');
      size.style.setProperty('font-weight', '800', 'important');
      size.style.setProperty('margin', '2px 0 4px', 'important');
      size.style.setProperty('justify-content', 'center', 'important');
      size.style.setProperty('justify-self', 'center', 'important');
      size.style.setProperty('text-align', 'center', 'important');
    }
    if (card.classList.contains('bc-compass-sister-you')){
      var youSize = card.querySelector('.bc-compass-sister-size');
      if (youSize){
        youSize.style.setProperty('font-size', '1.5rem', 'important');
        youSize.style.setProperty('margin', '4px 0 4px', 'important');
      }
    }
    var volume = card.querySelector('.bc-compass-sister-volume');
    if (volume){
      volume.style.setProperty('grid-column', '1', 'important');
      volume.style.setProperty('grid-row', '3', 'important');
      volume.style.setProperty('font-size', '0.78rem', 'important');
      volume.style.setProperty('display', 'inline-block', 'important');
      volume.style.setProperty('text-align', 'right', 'important');
      volume.style.setProperty('margin', '0', 'important');
      volume.style.setProperty('vertical-align', 'baseline', 'important');
      volume.style.setProperty('justify-self', 'end', 'important');
    }
    var band = card.querySelector('.bc-compass-sister-band');
    if (band){
      band.style.setProperty('grid-column', '2', 'important');
      band.style.setProperty('grid-row', '3', 'important');
      band.style.setProperty('font-size', '0.78rem', 'important');
      band.style.setProperty('display', 'inline-block', 'important');
      band.style.setProperty('text-align', 'left', 'important');
      band.style.setProperty('margin', '0 0 0 6px', 'important');
      band.style.setProperty('vertical-align', 'baseline', 'important');
      band.style.setProperty('letter-spacing', '0', 'important');
      band.style.setProperty('color', '#8a6850', 'important');
      band.style.setProperty('opacity', '0.85', 'important');
      band.style.setProperty('justify-self', 'start', 'important');
    }
    var conv = card.querySelector('.bc-compass-sister-conversion');
    if (conv){
      conv.style.setProperty('grid-column', '1 / -1', 'important');
      conv.style.setProperty('grid-row', '4', 'important');
      conv.style.setProperty('justify-content', 'center', 'important');
      conv.style.setProperty('justify-self', 'center', 'important');
      conv.style.setProperty('margin', '4px 0 0', 'important');
      conv.style.setProperty('gap', '6px', 'important');
      conv.style.setProperty('font-size', '0.6rem', 'important');
    }
    var comfort = card.querySelector('.bc-compass-sister-comfort, .bc-compass-sister-original');
    if (comfort){
      comfort.style.setProperty('grid-column', '1 / -1', 'important');
      comfort.style.setProperty('grid-row', '5', 'important');
      comfort.style.setProperty('font-size', '0.6rem', 'important');
      comfort.style.setProperty('justify-self', 'center', 'important');
      comfort.style.setProperty('margin', '4px 0 0', 'important');
    }
    if (card.classList.contains('bc-compass-sister-you')){
      card.style.setProperty('padding', '12px 14px', 'important');
      var youPointer = card.querySelector('.bc-compass-sister-pointer');
      if (youPointer) youPointer.style.setProperty('display', 'block', 'important');
      var youCheck = card.querySelector('.bc-compass-sister-check');
      if (youCheck) youCheck.style.setProperty('display', 'none', 'important');
    }
  }
  ...
  for (var j = 0; j < btns.length; j++){
    (function(btn){
      btn.addEventListener('click', function(ev){
        ...
        /* 联动效果: 点击波纹 + 切换 primary 后的波动 */
        btn.classList.add('bc-compass-sister-clicked');
        setTimeout(function(){ btn.classList.remove('bc-compass-sister-clicked'); }, 600);
...
function renderCompareOverlay(){
  ...
  /* 姐妹尺码定义: 相同体积. 使用 userOriginal (或 a 的体积) 作为基准 */
  var refVol = _userOriginalSize && BC.CUP_VOLUME_ML
    ? (BC.CUP_VOLUME_ML[_userOriginalSize.cup] || 0)
    : (BC.CUP_VOLUME_ML ? (BC.CUP_VOLUME_ML[a.cup] || 0) : 0);
  var va = refVol;
  var vb = refVol;
  ...
  /* 姐妹尺码体积永远相等 (volDiff=0), 在 UI 上明确说明 */
  var volDiffLabel = volDiff === 0 ? 'Equal' : (volDiff + ' mL');
  ...
  overlay.innerHTML =
    '<div class="bc-compass-compare-card">' +
      ...
      '<div class="bc-compass-compare-grid">' +
        ...
        '<div class="bc-compass-compare-row"><span>Cup volume</span><strong>' + volDiffLabel + '</strong></div>' +
...
function renderSisterSummary(sisters, currentBand, currentCup){
  ...
  /* 体积用 userOriginal (用户原始尺码的体积) — 这才是"等体积"原则的核心 */
  var userVol = (_userOriginalSize && BC.CUP_VOLUME_ML) ? (BC.CUP_VOLUME_ML[_userOriginalSize.cup] || 380) : 380;
  ...
  var summaryHtml =
    '<div class="bc-compass-summary-equal">' +
      ...
      '<span class="bc-compass-summary-equal-value">' + userVol + '</span>' +
    ...
    /* Task 6: 行动召唤 — 移除 Save my size 按钮, 只保留 Find your fit, 居中显示 */
    '<div class="bc-compass-actions bc-compass-actions-single">' +
...
/* 切换 sister primary — 平滑过渡动画 (fade-out → 重渲染 → fade-in + YOU 卡微弹) */
function switchSisterPrimary(band, cup){
  var sistersEl = document.getElementById('bc-compass-sisters');
  if (!sistersEl) return;
  var sisters = BC.getSisterSizes(band, cup) || [];
  /* 步骤 1: 旧卡淡出 + 轻微缩放 */
  sistersEl.classList.add('bc-compass-sisters-switching');
  setTimeout(function(){
    /* 步骤 2: 渲染新 7 张卡 */
    var shtml = '';
    for (var i = 0; i < sisters.length; i++){
    ...
    _compareSelection = [];
    renderCompareOverlay();
    /* 步骤 3: 强制重排后移除 switching 类, 触发淡入 + YOU 卡弹跳 */
    /* eslint-disable-next-line no-unused-expressions */
    sistersEl.offsetHeight;
    sistersEl.classList.remove('bc-compass-sisters-switching');
    sistersEl.classList.add('bc-compass-sisters-entered');
    /* 联动: 新 7 张卡顺序波动入场, 形成 "波纹" 效果 */
    var allNew = sistersEl.querySelectorAll('.bc-compass-sister');
    for (var wi = 0; wi < allNew.length; wi++){
      (function(card, idx){
        setTimeout(function(){
          card.classList.add('bc-compass-sister-switch-wave');
          setTimeout(function(){ card.classList.remove('bc-compass-sister-switch-wave'); }, 700);
        }, idx * 50);
      })(allNew[wi], wi);
    }
    var youCard = sistersEl.querySelector('.bc-compass-sister-you');
    if (youCard){
      youCard.classList.add('bc-compass-sister-you-pop');
      setTimeout(function(){ youCard.classList.remove('bc-compass-sister-you-pop'); }, 700);
    }
    setTimeout(function(){ sistersEl.classList.remove('bc-compass-sisters-entered'); }, 600);
  }, 180);
}
...
/* ──────────────────────────────────────────────────────────
   空状态 (empty state) 由 HTML 中的 7 张静态文字预览卡提供 (无 SVG)。
   active state 由 renderSisterComparator() 渲染, 调用 BC.getSisterSizes()。
   BC.getSisterSizes 的算法:
     - 用户尺码为中心 (primary=true)
     - 上下扩展各 3 个 sister: band±N, cup∓N
     - 共 7 张, 按 band 从小到大排序
   ────────────────────────────────────────────────────────── */

/* previewSisterDemo — 保留函数但默认 noop
   按钮已从 HTML 移除(用户希望没计算时不显示 preview 区域)
   保留函数以便测试或未来回退 */
function previewSisterDemo(){
  /* noop: 按钮已移除. 保留函数以备未来扩展. */
  if (typeof console !== 'undefined' && console.log){
    console.log('[bc-main] previewSisterDemo: noop (button removed)');
  }
}
...
function runCalculation(form){
  ...
  setTimeout(function(){
    try {
      ...
      if (!data){
        if (window.showToast) window.showToast('Form inputs not found.', 'error');
...
function wireAll(){
  ...
  /* 初始化 Sister Size Comparator — empty 状态直接由 HTML 提供 7 张静态文字预览卡 */
  resetSisterComparator();
