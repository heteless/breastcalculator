...
/* ─── Task 2: 5 区域尺码换算 (US → UK / EU / FR / AU) ───
   用于姐妹卡的副标签显示
   ── 换算公式 ──
   UK: band 相同, cup 字母按 UK_CUP_MAP (DDD→E, G→F ...)
   EU/FR: band 按 5cm 递增, 28→60, 30→65, 32→70, 34→75, 36→80, 38→85, 40→90
          cup 字母按 EU_CUP_MAP (DDD→F, G→G ...)
   AU: band 与 US 相同, cup 字母按 AU_CUP_MAP (DDD→E ...) */
function getSisterSizeConversion(band, cup){
  ...
  /* EU/FR band: 5cm 步进, 28→60, 30→65, 32→70, 34→75, 36→80, 38→85, 40→90, 42→95, 44→100 */
  var euBand = 60 + Math.round((band - 28) / 2) * 5;
  var frBand = euBand;       /* FR 同 EU */
