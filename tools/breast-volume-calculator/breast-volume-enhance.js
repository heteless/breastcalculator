/**
 * Breast Volume Calculator — Visual Enhancement Module
 * Injects SVG mind map, decorative dividers, image placeholders,
 * and scroll-triggered animations. Designed to complement existing
 * calculator functionality without modifying its logic.
 */
(function () {
  'use strict';
  if (window.__bvEnhanced) return;
  window.__bvEnhanced = true;

  // ==========================================================================
  // 1. SVG Mind Map — Breast Volume Category Visualization
  // ==========================================================================
  function createMindMapSVG() {
    /* Central node + 6 branch nodes in a flower/petal layout.
     * Warm rose-gold palette with soft gradients. */
    var svg = '' +
...
'</div>';
  ...
  // ==========================================================================
  // 3. Image Placeholder (elegant card with image slot)
  // ==========================================================================
...
'</div>';
  ...
  function initScrollAnimation() {
    if (typeof IntersectionObserver === 'undefined') {
      document.querySelectorAll('.bv-animate-in').forEach(function (el) {
        el.style.opacity = '1';
      });
      return;
    }
  ...
  // ==========================================================================
  // 5. Injection logic — add elements to the page
  // ==========================================================================
  function injectEnhancements() {
    var main = document.querySelector('main');
    if (!main) return;

    // Inject mind map after the interpretation section
    var interpretation = main.querySelector('.interpretation-v2');
    if (interpretation) {
      var mindmapWrapper = document.createElement('div');
      mindmapWrapper.className = 'bv-mindmap-wrapper bv-animate-in bv-delay-3';
      mindmapWrapper.innerHTML =
        ...
        createMindMapSVG();
      interpretation.parentNode.insertBefore(mindmapWrapper, interpretation.nextSibling);
    }

    // Inject image placeholder before how-to-measure section
    var howToMeasure = main.querySelector('.how-to-measure');
    if (howToMeasure) {
      var placeholderDiv = document.createElement('div');
      placeholderDiv.innerHTML = createImagePlaceholder();
      howToMeasure.parentNode.insertBefore(placeholderDiv.firstElementChild, howToMeasure);

      // Inject divider before image placeholder
      var dividerDiv = document.createElement('div');
      dividerDiv.innerHTML = createDivider();
      howToMeasure.parentNode.insertBefore(dividerDiv.firstElementChild, howToMeasure.previousElementSibling);
    }

    // Add step classes to how-to-measure headings
    if (howToMeasure) {
      var h3s = howToMeasure.querySelectorAll('h3');
      for (var i = 0; i < h3s.length; i++) {
        h3s[i].classList.add('step-' + (i + 1));
      }
    }

    // Add animation classes to tool-card
    var toolCard = main.querySelector('.tool-card');
    if (toolCard) {
      toolCard.classList.add('bv-animate-in');
    }

    // Add animation classes to interpretation, result, and FAQ
    var elements = main.querySelectorAll('.interpretation-v2, .result-card-v2, .how-to-measure, .tool-faq-v2');
    elements.forEach(function (el, i) {
      el.classList.add('bv-animate-in');
      if (i < 4) el.classList.add('bv-delay-' + (i + 1));
    });
  ...
  // Retry a couple times for late dynamic content
  setTimeout(injectEnhancements, 80);
  setTimeout(injectEnhancements, 300);
})();
