#!/usr/bin/env python3
"""
Insert Step 2 (LocalStorage history) logic into calculator.js.

calculator.js is a minified IIFE produced by split-script.js. It contains
the same initSizeCalculator as bra-calculator-enhance.js (with choice stage
+ setTimeout). We make 2 surgical insertions:
  1. New history helper functions + init calls before the final `})();`
  2. Inside initSizeCalculator's setTimeout, store result and render history
"""

import sys

PATH = r"d:\DevProject\breastcalculator\calculator.js"
...
needle1 = "})();"
last_idx = src.rfind(needle1)
if last_idx < 0:
    sys.stderr.write("[end-iife] NOT FOUND\n")
    sys.exit(1)
src = src[:last_idx] + NEW_HELPERS + needle1
print(f"[end-iife] inserted helpers before final )();")

# 2) Inside initSizeCalculator, after revealDetails is wired and before setTimeout closes.
needle2 = "var skipBtn = document.getElementById('size-choice-skip');if (skipBtn) skipBtn.addEventListener('click',revealDetails);}},2500);});"
replacement2 = "var skipBtn = document.getElementById('size-choice-skip');if (skipBtn) skipBtn.addEventListener('click',revealDetails);form.__lastResult=result;try{var __hr=document.getElementById('bc-history');if(__hr)bcRenderHistory(__hr,form);}catch(_){}}},2500);});"
src = replace_once(src, needle2, replacement2, "initSizeCalc-end")

with open(PATH, 'w', encoding='utf-8') as f:
    f.write(src)

print(f"[step2-calc] Wrote {len(src)} chars to {PATH}")
